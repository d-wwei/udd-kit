...
传统软件研发是公司主导的：用户提反馈，公司筛选排期，公司决定做什么。用户有表达权，但没有实现权。

AI 改变了这个等式。有了 Coding Agent，每个用户都拥有了即时造物和改造软件的能力，不再需要等待公司的路线图。这不只是效率提升，而是**软件演进控制权的根本性转移**。

我们把这个转变称为 **用户主导开发（User-Directed Development，UDD）**：

1. 用户在自己的真实场景中遇到问题
2. 背后的 Agent 收集证据，尝试安全的本地修复
3. 成功的修复回流到上游，成为可复用的改进
4. 软件从边缘演进，而不只是从中心

但这个闭环不会自己闭合。用户的 Agent 在本地修好了一个问题，这个知识留在本地。上游发了一个 fix，用户不知道它解决了自己的问题。修复破坏了什么，没有回滚。修复有价值，没有回馈路径。

**UDD Kit 就是闭合这个循环的编排层。** 它把故障检测、上游感知、本地修复、验证、上游贡献串成一条自动化链路。

> *"软件演进不应停留在'用户反馈，公司决定'的单向模式，而应转向'用户决定方向，Agent 执行建造，平台负责边界治理'的新范式。"*

延伸阅读：[产品的未来：从"以用户为中心"到"用户主导开发"（微信公众号）](https://mp.weixin.qq.com/s/UQAgRGJnpOi5nJ19Bu6qBA)

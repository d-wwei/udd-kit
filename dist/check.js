...
export async function detectCurrentVersion(cwd, manifest) {
    const source = manifest.currentVersionSource;
    if (source.type === "literal" && source.value)
        return normalizeVersion(source.value);
    const filePath = resolveFromCwd(cwd, source.path);
    if (!filePath)
        throw new Error(`Version source ${source.type} requires a path.`);
    ...
    if (source.type === "pyproject.toml") {
        ...
        if (!match)
            throw new Error("pyproject.toml missing version.");
    ...
    if (source.type === "file") {
        ...
        if (!match)
            throw new Error(`Could not find key ${source.key}.`);
        return normalizeVersion(match[1]);
    }
    throw new Error(`Unsupported version source ${source.type}.`);
}
export async function checkForUpdates(ctx, manifest, options = {}) {
    ...
    let highlights = [];
    let compareUrl;
    const shouldRefresh = !cacheEntry?.lastCheckedAt ||
...
export async function ignoreUpdateVersion(manifest, version, options = {}) {

...
async function git(args, cwd, exec) {
    if (exec)
 ...
 */
export async function detectIdentity(cwd, manifest, remoteName = "origin", exec) {
 ...
 */
async function resolveStrategy(cwd, manifest, explicitStrategy, remoteName, exec) {
 ...
 */
async function ensureUpstreamRemote(cwd, upstreamRepo, upstreamRemoteName, exec) {
 ...
 */
async function detectChanges(cwd, manifest, exec) {
    const raw = await git(["status", "--short"], cwd, exec);
    if (!raw)
        return [];
    const internalPaths = new Set([".udd/", manifest.state?.path, manifest.audit?.path]
        .filter((v) => Boolean(v)));
    return raw
        .split(/\r?\n/)
        .filter(Boolean)
 ...
 */
export async function contribute(adapter, manifest, ctx, options = {}) {
    ...
    const protectedHits = changedFiles.filter((file) => protectedPaths.some((pattern) => file.includes(pattern.replace("*", ""))));
    ...
    if (auth) {
        if (resolved.identity === "external" && resolved.originRepo) {
            ...
            const pr = await createGithubPullRequest(upstreamRepo, {
...
function buildPrBody(changedFiles, diffStat, verification, identity, originRepo) {
    const lines = [
        "## Changes",
    ...
    lines.push("## Contributor", `- Identity: **${identity}**`, originRepo ? `- Fork: \`${originRepo}\`` : "", `- Generated by [UDD Kit](https://github.com/d-wwei/udd-kit)`);

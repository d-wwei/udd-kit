...
export async function prepareIssueDraft(ctx, manifest, options = {}) {
    ...
    const environment = {
        ...
        cwd: "[PATH]",
        ...Object.fromEntries(Object.entries(ctx.metadata ?? {}).map(([key, value]) => [key, value == null ? "" : String(value)]))
    };
...
export async function submitIssue(ctx, draft, auth, options = {}) {
    ...
    const result = await createGithubIssue(`${draft.owner}/${draft.repo}`, draft.title, draft.body, auth, options.fetchImpl ?? fetch);
    return { url: result.html_url ?? "" };
}
async function collectLogAttachments(ctx, manifest) {
    const attachments = [];
    for (const logPath of ctx.logs ?? []) {
        if (isExcluded(logPath, manifest))
            continue;
        try {
...
function isExcluded(filePath, manifest) {

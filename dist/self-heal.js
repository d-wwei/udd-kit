...
function validationLines(verification) {
    return verification.stages.flatMap((stage) => stage.steps.map((step) => `${stage.stage}:${step.name}:${step.ok ? "ok" : "failed"}`));
}
async function persistState(adapter, manifest, ctx, state, patch) {
    const next = {
        ...state,
        ...patch
    };
...
async function escalateIssue(adapter, manifest, ctx, diagnosis, options, summary) {
...
export async function analyzeIncident(adapter, manifest, options = {}) {
...
export async function planHealing(adapter, manifest, options = {}) {
    const { confirm, auth: _auth, submitIssueOnEscalation: _submit, createPr: _createPr, ...overrides } = options;
    const incident = await collectIncidentContext(adapter, manifest, { ...overrides, confirm });
...
export async function healIncident(adapter, manifest, options = {}) {
    ...
    try {
        ...
        if (!verification.ok) {
            ...
            const result = await escalateIssue(adapter, manifest, incident, diagnosis, options, `Verification failed after ${plan.strategy} at ${verification.failedStep ?? "unknown step"}.`);
            await persistState(adapter, manifest, incident, state, {
                lastHeal: {
                    status: "escalated",
                    strategy: plan.strategy,

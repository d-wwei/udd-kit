...
export async function prepareContributionDraft(ctx, manifest, options = {}) {
...
export async function submitContribution(ctx, draft, auth, options = {}) {
    if (!draft.allowed) {
    ...
    if (!confirmed)
        throw new Error("Contribution submission cancelled by user.");
    ...
    if (options.createPr !== false) {
        const pr = await createGithubPullRequest(`${draft.owner}/${draft.repo}`, {
            title: draft.prTitle,
            body: draft.prBody,
            head: draft.branchName,
            base: options.baseBranch ?? "main"
        }, auth, options.fetchImpl ?? fetch);
        prUrl = pr.html_url;
    }
    return {

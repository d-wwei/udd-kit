import type { HostError, UpstreamFixMatch } from "./types.js";
...
function scoreHighlight(
  ...
  if (!errorTokens.length) return 0;
  ...
  const sharedCodes = errorCodes.filter((c) =>
    highlightCodes.some((hc) => hc.toLowerCase() === c.toLowerCase())
  );
...
function confidenceLevel(score: number): "low" | "medium" | "high" | undefined {
...
export function matchChangelogToError(
  error: HostError,
  highlights: string[]
): UpstreamFixMatch | undefined {
  ...
  if (!errorTokens.length || !highlights.length) return undefined;

...
export async function fetchLatestTag(
  ...
  if (!tags[0]) throw new Error("No tags found.");
  return tags[0];
}
...
export async function fetchHighlights(params: {
  ...
  apiBaseUrl?: string;
  cwd: string;
}): Promise<{ highlights: string[]; compareUrl?: string }> {
  const {
    repo,
    currentVersion,
    latestVersion,
  ...
  if (changelogSource.type === "compare_commits") {
    try {
      const res = await fetchImpl(
        `${apiBaseUrl}/repos/${repo}/compare/v${currentVersion}...v${latestVersion}`,
        { headers: getHeaders(token) }
      );
      if (!res.ok) throw new Error(String(res.status));
      const compare = (await res.json()) as GithubCompare;
